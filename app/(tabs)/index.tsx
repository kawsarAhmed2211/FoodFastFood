...
export default function Index() {
  return (
    <SafeAreaView className={"flex-1 bg-white"}>
        {/*<ScrollView>*/}
            {/* reason why we use scollview is so that everything inside is scrollable and other components do not overlap*/}
            {/*flatlist is like .map in javascript*/}
            <FlatList
                data={offers}
                              ...
                              {({ pressed }) => (
                                  <Fragment>
                                      ...
                                      <View className={cn("offer-card__info", isEven? "pl-10": "pr-10")}>
                                          <Text className={"h1-bold text-white-100 leading-tight"}>{item.title}</Text>
